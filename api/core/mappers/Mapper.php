...
class Mapper
{
    ...
    public function getByName($sCrmId, $iControllerId = '', $iCompanyId = '') {}
...
EOQ;
    ...
    {
//        $iResult = $this->oDb->sql_exec_for_binding($sSql, $aBnd);

        foreach ($aBnd as $key => $value) {
            $sSql = str_replace(':'.$key, '\''.$value.'\'', $sSql);
        }

        $bResult = $this->oDb->execute($sSql);
     ...
     * @return type
     */
    public function update($sSql, $aBnd, $sTableName = '')
    {
        foreach ($aBnd as $key => $value) {
            $sSql = str_replace(':'.$key, '\''.$value.'\'', $sSql);
        }
     ...
     * @return type
     */
    public function delete($sSql, $aBnd, $sTableName = '')
    {
        foreach ($aBnd as $key => $value) {
            $sSql = str_replace(':'.$key, '\''.$value.'\'', $sSql);
        }
    ...
    {
//        $bResult = $this->oDb->sql_open_for_binding($sSql, $aBnd);

        foreach ($aBnd as $key => $value) {
            $sSql = str_replace(':'.$key, '\''.$value.'\'', $sSql);
        }

        $aResult = $this->oDb->select($sSql);
