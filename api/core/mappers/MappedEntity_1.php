...
//    abstract function defineMapper();
    ...
    function __construct($oDb)
    {
        ...
        /* @var $oTvTopology TvTopology */
        $this->oTvTopology = TvTopology::getInstance($this->oDb);
    ...
    {
        $iId     = $dto->id;
        $iControllerId = $dto->controllerId;

        $aRs = $this->oMapper->getById($iId, $iControllerId);
        $this->init($aRs);
        return;
    }

    public function loadByCrmId(LoadByCrmIdDTO $dto)
    {
        $sCrmId        = $dto->crmId;
        $iControllerId = $dto->controllerId;
        $iCompanyId    = $dto->companyId;
        $sControllerType = $dto->controllerType;

        $aRs = $this->oMapper->getByCrmId($sCrmId, $iControllerId, $iCompanyId, $sControllerType);
        $this->init($aRs);
        return;
    }

    public function loadByExternalId(LoadByExternalIdDTO $dto)
    {
        $iExternalId = $dto->externalId;
        $iControllerId = $dto->controllerId;

        $aRs = $this->oMapper->getByExternalId($iExternalId, $iControllerId);
        $this->init($aRs);
        return;
    }

    public function loadByStbId(LoadByStbIdDTO $dto)
    {
        $iStbId  = $dto->stbId;
        $aRs = $this->oMapper->getByStbId($iStbId);
        $this->init($aRs);
        return;
    }

    public function loadByESPV(LoadByESPVDTO $dto)
    {
        ...
        $aRs = $this->oMapper->getByESPV($idEmpresa, $idServicio, $idProducto, $idVenta);
        $this->init($aRs);
        return;
    }

    public function loadByESPVC(LoadByESPVCDTO $dto)
    {
        $idEmpresa  = $dto->idEmpresa;
        $idServicio = $dto->idServicio;
        $idProducto = $dto->idProducto;
        ...
        $aRs = $this->oMapper->getByESPVC($idEmpresa, $idServicio, $idProducto, $idVenta, $idCliente);
        $this->init($aRs);
        return;
    }

    public function loadByESPVP(LoadByESPVDTO $dto)
    {
        $idEmpresa  = $dto->idEmpresa;
        $idServicio = $dto->idServicio;
        $idProducto = $dto->idProducto;
        ...
        $aRs = $this->oMapper->getByESPVP($idEmpresa, $idServicio, $idProducto, $idVenta);
        $this->init($aRs);
        return;
    }

    public function loadByDeviceId(LoadByDeviceId $dto)
    {
        $additionalId = $dto->additionalId;
        $unitAddress  = $dto->unitAddress;
        $serialNumber = $dto->serialNumber;
        $smartCard    = $dto->smartCard;

        $aRs = $this->oMapper->getByDeviceId($unitAddress, $serialNumber, $smartCard, $additionalId);
        $this->init($aRs);
        return;
    }
    ...
    public function create()
    {
        ...
        return ($iResult == 1)
                ? '' : "MAPPED_ENTITY_FAILED_TO_CREATE_[".get_class($this)."]_INSTANCE_THROUGH_MAPPEDENTITY | Rows affected: [{$iResult}]";
    }
    ...
    public function modify()
    {
        ...
        return ($iResult == 1)
                ? '' : "MAPPED_ENTITY_FAILED_TO_MODIFY_[".get_class($this)."]_INSTANCE_THROUGH_MAPPEDENTITY | Rows affected: [{$iResult}]";
    }
    ...
    public function remove()
    {
        ...
        return ($iResult == 1)
                ? '' : "MAPPED_ENTITY_FAILED_TO_REMOVE_[".get_class($this)."]_INSTANCE_THROUGH_MAPPEDENTITY | Rows affected: [{$iResult}]";
    }
